...
def _build_hitl_resume_state(result: dict) -> dict:
    ...
    return {
        ...
        "selected_documents": result.get("selected_documents") or trace.get("selected_documents") or [],
...
def _initial_state(
    ...
    *,
    ...
    active_selected_documents = selected_documents or ctx.selected_documents
    return {
        "question": question,
        "query": question,
        "context": "",
        ...
        "sub_questions": None,
        "selected_documents": active_selected_documents,
...
def retrieve_initial(state: RAGState) -> RAGState:
    ...
    if selected_documents:
        detail = "限定文档：" + "；".join(selected_documents[:3])
    _emit(state, "🔍", "正在检索知识库...", detail)
...
def synthesis(state: RAGState) -> RAGState:
    ...
    rag_trace = {
        **original_trace,
        ...
        "document_filter_enabled": bool(selected_documents),
        "selected_documents": selected_documents,
    }
...
def _state_from_resume(
    ...
    selected_documents = resume_state.get("selected_documents") or ctx.selected_documents
    ...
    state = _initial_state(refined_question, ctx, selected_documents=selected_documents)
    state.update({
        "query": refined_question,
